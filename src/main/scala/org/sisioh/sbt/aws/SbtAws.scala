...
object SbtAws extends SbtAwsS3 with SbtAwsEB with SbtAwsCfn {
  ...
  private[aws] def newCredentialsProvider(profileName: Option[String]) = {
  ...
  private[aws] def createClient[A <: AmazonWebServiceClient](serviceClass: Class[A], region: Region, profileName: Option[String]): A = {
