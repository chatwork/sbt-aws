...
import org.sisioh.aws4s.eb.model.{
  DescribeConfigurationSettingsRequestFactory,
  DescribeEventsRequestFactory,
  S3LocationFactory,
  ValidateConfigurationSettingsRequestFactory
}
import sbt.Keys._
import sbt._
...
trait SbtAwsEb
  ...
  private[eb] def wait[T](client: AWSElasticBeanstalkClient)(call: => T)(break: (T) => Boolean)(
...
Def.task {
  ...
  private[eb] def validateConfigurationSettings(
  ...
  private[eb] def ebDescribeEvents(client: AWSElasticBeanstalkClient,
                                   applicationName: String,
                                   envrionmentName: Option[String],
                                   templateName: Option[String])(implicit logger: Logger) = {
  ...
  private[eb] def ebDescribeConfigurationSettings(
      client: AWSElasticBeanstalkClient,
      applicationName: String,
      environmentName: Option[String],
      templateName: Option[String])(implicit logger: Logger) = {
    logger.debug(
      s"describe configurationSettings start: $applicationName, $environmentName, $templateName")
    ...
    logger.debug(
      s"describe configurationSettings finish: $applicationName, $environmentName, $templateName")
    result
  }

  private[eb] def ebListAvailableSolutionStacks(client: AWSElasticBeanstalkClient)(
      implicit logger: Logger) = {
    client.listAvailableSolutionStacksAsTry().map { result =>
      result.getSolutionStackDetails.asScala.map { detail =>
        logger.info(
          s"${detail.getSolutionStackName}, ${detail.getPermittedFileTypes.asScala.mkString("{ ", ", ", " }")}")
        detail
      }
    }
  }
