  ...
  private[eb] def ebCreateEnvironment(client: AWSElasticBeanstalkClient,
    ...
    ebDescribeEnvironment(client, applicationName, environmentName).flatMap {
      _.map { _ =>
        ...
        throw AlreadyExistsException(s"The application already exists.: $applicationName, $environmentName")
      }.getOrElse {
        ...
        val baseRequest = CreateEnvironmentRequestFactory
          ...
          .withTags(tags: _*)
        val request = if (tier.fold(false)(_.getType == "WebServer"))
      ...
      _.map {
        environment =>
          if (EnvironmentStatus.valueOf(environment.getStatus) == EnvironmentStatus.Ready) {
            logger.info(s"update environment start: $applicationName, $environmentName, $description, $versionLabel, $tier, $solutionStackName, $configurationTemplateName, $configurationOptionSettings, $optionSpecifications")
            val request = UpdateEnvironmentRequestFactory
              ...
              .withOptionSettings(configurationOptionSettings: _*)
              .withOptionsToRemove(optionSpecifications: _*)
            val result = client.updateEnvironmentAsTry(request).map { r =>
              EnvironmentDescriptionFactory
                .create()
                .withApplicationName(r.getApplicationName)
                .withCNAME(r.getCNAME)
            ...
            throw EnvironmentNotReadyException(s"The environment is not ready: $applicationName, $environmentName")
          }
      }.getOrElse {
        logger.warn(s"not found environment: $applicationName, $environmentName")
        throw NotFoundException(s"The environment is not found: $applicationName, $environmentName")
